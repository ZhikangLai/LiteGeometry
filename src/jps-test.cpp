...
int main() {
	...
	JPSPathFinder1 _JPSPathFinder1(SCSet, EFSet);
	auto start1 = std::chrono::high_resolution_clock::now();
	const auto& bestPaths1 = _JPSPathFinder1.JPSGraphSearch(startPoint, endPoint1);
	auto end1 = std::chrono::high_resolution_clock::now();

	auto duration1 = std::chrono::duration_cast<std::chrono::milliseconds>(end1 - start1);
	std::cout << "JPSPathFinder1 took: " << duration1.count() << " milliseconds" << std::endl;

	JPSPathFinder2 _JPSPathFinder2(SCSet, EFSet);
	auto start2 = std::chrono::high_resolution_clock::now();
	const auto& bestPaths2 = _JPSPathFinder2.JPSGraphSearch(startPoint, endPoint1);
	auto end2 = std::chrono::high_resolution_clock::now();

	auto duration2 = std::chrono::duration_cast<std::chrono::milliseconds>(end2 - start2);
	std::cout << "JPSPathFinder2 took: " << duration2.count() << " milliseconds" << std::endl;
